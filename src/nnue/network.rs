...
impl Accumulator {
    ...
    pub fn apply_feature_updates(&mut self, adds: &[usize], removes: &[usize], net: &Network) {
        ...
        unsafe {
            ...
            for i in (0..HIDDEN_SIZE).step_by(16) {
                let mut v = _mm256_load_si256(dst.add(i) as *const __m256i);

                for &idx in adds {
                    let w = _mm256_load_si256(net.feature_weights[idx].vals.as_ptr().add(i) as *const __m256i);
                ...
                _mm256_store_si256(dst.add(i) as *mut __m256i, v);
            }
        }
    }
}